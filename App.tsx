...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<'dashboard' | 'marketplace' | 'finance' | 'ai' | 'predictor' | 'sell'>('dashboard');
        ...
        <section className="py-12 bg-white border-y border-slate-100">
           <div className="max-w-7xl mx-auto px-4 text-center">
              <button 
                ...
                className="bg-green-600 text-white px-10 py-5 rounded-3xl text-xl font-black shadow-2xl shadow-green-200 hover:bg-green-700 hover:scale-105 active:scale-95 transition-all"
              >
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24">
      <header className="bg-white border-b border-slate-100 sticky top-0 z-40 px-4 py-3 flex flex-col gap-3 shadow-sm">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white font-black transition-all duration-500 ${role === 'farmer' ? 'bg-green-600' : role === 'buyer' ? 'bg-blue-600' : 'bg-purple-600'}`}>
              {role[0].toUpperCase()}
            </div>
            <span className="font-black tracking-tight text-lg">FosolSetu</span>
          </div>

          {/* New Role Switcher (Middle) */}
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner">
            {(['farmer', 'buyer', 'seller'] as AppRole[]).map((r) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-tight transition-all duration-300 ${
                  role === r 
      ...
      <main className="max-w-md mx-auto px-4 pt-6">
        ...
        {view === 'sell' && <SellerPortal lang={lang} />}
      </main>

      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-100 px-6 py-4 flex justify-between items-center z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        ...
        {role === 'seller' ? (
           <NavIcon active={view === 'sell'} onClick={() => setView('sell')} icon="M12 4v16m8-8H4" />
        ) : (
...
const NavIcon: React.FC<{ active: boolean; onClick: () => void; icon: string }> = ({ active, onClick, icon }) => (
  <button 
    onClick={onClick}
    className={`p-3 rounded-2xl transition-all ${active ? 'bg-slate-900 text-white shadow-lg scale-110' : 'text-slate-400 hover:bg-slate-50'}`}
  >
